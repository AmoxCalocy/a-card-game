...
- 运行前需在仓库 Secrets 配置 `UNITY_LICENSE`（序列化 license 内容）、`UNITY_EMAIL`、`UNITY_PASSWORD`；缺失时工作流会立即报错提示。工作流已默认使用 Node.js 24（`FORCE_JAVASCRIPT_ACTIONS_TO_NODE24=true`）以规避 Node 20 弃用警告。
